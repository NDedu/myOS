...
# Start or stop screen recording (clicking the Waybar indicator stops it)
# Usage: screenrecording.sh [--with-desktop-audio] [--with-microphone-audio] [--with-webcam] [--webcam-device=<device>] [--resolution=<size>] [--stop-recording]
#
# Env: SCREENRECORD_USE_PORTAL=true skips the built-in slurp picker and
...
# for monitors driven by external GPUs, and window capture. Off by default
# because the portal path can fail EGL
# DMA-BUF modifier import on some configurations, leaving recording unable to
# start.
#
# Env: SCREENRECORD_DEBUG=true appends gpu-screen-recorder's stderr (and
# the picker target it was launched with) to waybar-screenrecord.log in
# $XDG_RUNTIME_DIR, to attach when reporting a capture failure.
...
RECORDING_FILE="$RUNTIME_DIR/waybar-screenrecord-filename"
LOG_FILE=$([[ ${SCREENRECORD_DEBUG:-false} == "true" ]] && echo "$RUNTIME_DIR/waybar-screenrecord.log" || echo "/dev/null")
