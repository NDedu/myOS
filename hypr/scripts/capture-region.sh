...
# Pick a screen region over frozen screen content (used by screenshot.sh)
# Usage: capture-region.sh [region|windows|smart|fullscreen] [--keep-freeze] [--match-monitor]
#        capture-region.sh --take-fullscreen | --take-window | --select-window <next|prev|left|right|up|down>
...
FULLSCREEN_MARKER="${XDG_RUNTIME_DIR:-/tmp}/hypr-capture-region-fullscreen"
WINDOW_MARKER="${XDG_RUNTIME_DIR:-/tmp}/hypr-capture-region-window"
...
# surface (modules/bindings/utilities.lua) invoke these modes. The
